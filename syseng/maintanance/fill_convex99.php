...
unset($base_rec);
// find start values 
$sql = ' SELECT `RoomId`, `NumberofPeople`, `EventTime` FROM `roomoccupancy` WHERE
			EventTime = (SELECT max(EventTime) FROM roomoccupancy 
				WHERE DATEDIFF( NOW(), `EventTime` ) >= '.$WeeksToCount.'*7)  
	ORDER BY 3 DESC, 1 ASC LIMIT '.$roomCount;
foreach ($pdo->query($sql, PDO::FETCH_ASSOC) as $row) {
	$RoomId = $row['RoomId'];
